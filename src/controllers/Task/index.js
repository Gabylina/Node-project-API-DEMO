...
// Helper to extract numeric ID from params
...
export async function show(req, res, next) {
  try {
    ...
    if (projectId === null || taskId === null) {
      return res.status(400).json({ message: 'IDs de proyecto o tarea no válidos.' });
    }
...
export async function changeStatus(req, res, next) {
  try {
    ...
    const inputStatusNormalized = String(status).toLowerCase().replace(/_/g, '-');
    const validStatuses = ['pending', 'in-progress', 'completed'];
    if (!validStatuses.includes(inputStatusNormalized)) {
      return res.status(400).json({ message: 'El status proporcionado no es válido. Debe ser "pending", "in-progress", o "completed".' });
    }
