...
export default {
  ...
  channels: {
    ...
    single: {
      driver: 'single',
      path: 'logs/laravel.log', // Original: storage_path('logs/laravel.log')
    ...
    papertrail: {
      driver: 'monolog',
      level: process.env.LOG_LEVEL ?? 'debug',
      handler: process.env.LOG_PAPERTRAIL_HANDLER ?? 'SyslogUdpHandler', // Original: SyslogUdpHandler::class
      handler_with: {
      ...
      processors: ['PsrLogMessageProcessor'], // Original: PsrLogMessageProcessor::class
    },

    stderr: {
      driver: 'monolog',
      level: process.env.LOG_LEVEL ?? 'debug',
      handler: 'StreamHandler', // Original: StreamHandler::class
      handler_with: {
        stream: 'stderr', // Original: 'php://stderr'
      },
    ...
    syslog: {
      driver: 'syslog',
      level: process.env.LOG_LEVEL ?? 'debug',
      facility: process.env.LOG_SYSLOG_FACILITY ?? 'LOG_USER', // Original: LOG_USER
      replace_placeholders: true,
    },
    ...
    null: {
      driver: 'monolog',
      handler: 'NullHandler', // Original: NullHandler::class
    },
